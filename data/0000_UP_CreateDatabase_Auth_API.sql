  CREATE DATABASE [AuthDB]
   CONTAINMENT = NONE
   ON  PRIMARY 
  ( NAME = N'AuthDB', FILENAME = N'/var/opt/mssql/data/AuthDB.mdf' , SIZE = 102400KB , FILEGROWTH = 65536KB )
   LOG ON 
  ( NAME = N'AuthDB_log', FILENAME = N'/var/opt/mssql/data/AuthDB_log.ldf' , SIZE = 8192KB , FILEGROWTH = 65536KB )
   WITH LEDGER = OFF
  GO
  ...
  GO
  ALTER DATABASE [AuthDB] SET AUTO_CREATE_STATISTICS ON(INCREMENTAL = OFF)
  GO
  ...
  USE [AuthDB]
  GO
  IF NOT EXISTS (SELECT name FROM sys.filegroups WHERE is_default=1 AND name = N'PRIMARY') ALTER DATABASE [AuthDB] MODIFY FILEGROUP [PRIMARY] DEFAULT
  GO